...
class TaskScheduler:
    ...
    def _generate_tasks(self):
        ...
        for data_flag, model_flag, noise_cfg, split in product(
            self._get_data_flags(), space['model_flags'], space['noise_conditions'], space['splits']
        ):
            # Check if final Zarr store already exists
            output_filename = f"dataset-{data_flag}_model-{model_flag}_noise-{noise_cfg['name']}_split-{split}.zarr.zip"
    ...
    def _start_task(self, task: Task):
        ...
        try:
            ...
            log_handle.close()
            self.task_queue.appendleft(task) # Re-queue the task
            self.available_gpus.append(gpu_id) # Release GPU
    ...
    def _check_running_tasks(self):
        ...
        for task_id, task in list(self.running_tasks.items()):
            ...
            elif time.time() - task.start_time > self.scheduler_cfg['task_timeout']:
    ...
    def _handle_failed_task(self, task: Task):
        ...
        if task.retries <= self.scheduler_cfg['max_retries']:
            ...
            task.status = "pending"
            self.task_queue.appendleft(task) # Add to the front for immediate retry
        else:
    ...
    def _run_preflight_check(self):
        ...
        gpu_id = self.scheduler_cfg['available_gpus'][0]
        cmd = [sys.executable, "preflight_checker.py", "--config", self.config_path, "--gpu-id", str(gpu_id)]
        try:
            # Using run with check=True is simpler for a blocking prerequisite
