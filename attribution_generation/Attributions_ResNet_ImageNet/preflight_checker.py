...
class BatchSizeFinder:
    ...
    def _setup_dataset_and_model(self, model_flag: str, data_flag: str):
        self.logger.info(f"Setting up dataset '{data_flag}' and model '{model_flag}' for preflight...")
        ...
        if not dataset_path.exists():
             self.logger.critical(f"Dataset path required for preflight not found: {dataset_path}")
             sys.exit(1)
        ...
        if len(full_dataset) < subset_size:
            self.logger.warning(f"Full dataset size ({len(full_dataset)}) is smaller than subset_size ({subset_size}). Will use full dataset for preflight.")
    ...
    def run(self):
        self.logger.info("--- Starting preflight: Batch size finder ---")
        final_results = {}
        ...
        for model_flag in self.search_space['model_flags']:
            ...
            pbar = tqdm(method_configs, desc=f"Testing model {model_flag}")
            for method_cfg in pbar:
                ...
                if method_name in self.preflight_cfg.get('exempt_methods', {}):
                    ...
                    self.logger.info(f"'{method_unique_key}' is exempt. Setting batch size to {exempt_bs}.")
                ...
                self.logger.info(f"Optimal batch size for '{method_unique_key}': {safe_bs}")
        ...
        self.logger.info(f"--- Preflight completed. Results saved to {results_path} ---")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XAI experiment preflight script.")
    parser.add_argument('--config', type=str, default="config.yml", help='Path to main experiment config file.')
    parser.add_argument('--gpu-id', type=int, required=True, help='GPU ID to use for checking.')
