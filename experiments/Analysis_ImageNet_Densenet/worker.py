...
class ExperimentAnalyzer:
    ...
    def run(self):
        ...
        pt_data_root = Path(self.config['pt_data_root'])
        p = self.task_params
        noise_dir = pt_data_root / f"model-{p['model']}_noise-{p['noise']}"

        self.logger.info(f"Loading base .pt data from: {noise_dir}")
        try:
            images = torch.load(noise_dir / "images.pt")
            labels = torch.load(noise_dir / "labels.pt").squeeze()
            original_preds_logits = torch.load(noise_dir / "model_predictions.pt")
            original_preds = torch.argmax(original_preds_logits, dim=1)
            num_samples = len(images)
            self.logger.info(f"Base data loaded ({num_samples} samples).")
        except FileNotFoundError as e:
            self.logger.critical(f"Failed to load base .pt files: {e}. Have you run generate_densenet_explanations.py?")
            exit(1)

        all_attributions = {}
        attr_dir = noise_dir / "attributions"
        attr_files = list(attr_dir.glob("*.pt"))
        if not attr_files:
            self.logger.critical(f"No attribution .pt files found in {attr_dir}.")
            exit(1)
            
        self.logger.info(f"Loading {len(attr_files)} attribution methods...")
        for pt_file in tqdm(attr_files, desc="Loading attributions"):
            method_name = pt_file.stem
            all_attributions[method_name] = torch.load(pt_file)
        ...
        fill_tensors = self._prepare_fill_tensors(noise_dir / "means", original_preds)
    ...
    def _prepare_fill_tensors(self, means_pt_dir: Path, original_preds: torch.Tensor):
        self.logger.info(f"Preparing fill tensors from .pt means dir: {means_pt_dir}")
        
        class_means = {}
        global_mean = torch.load(means_pt_dir / "overall.pt")
        
        for pt_file in means_pt_dir.glob("class_*.pt"):
            try:
                class_id = int(pt_file.stem.split('_')[1])
                class_means[class_id] = torch.load(pt_file)
            except Exception as e:
                self.logger.warning(f"Could not load mean file {pt_file}: {e}")
